...
<head>
    ...
    <title>Öğrenci Paneli - @yield('title')</title>
...
<nav class="sidebar">
    <div class="profile-area">
        ...
        {{-- YENİ: Öğrenci Maili Buraya Eklendi --}}
        <p class="mb-0 text-white-50" style="font-size: 0.85rem; word-break: break-all;">{{ Auth::user()->email }}</p>
    </div>
    ...
    <div class="menu-area">
        ...
        <a href="{{ route('reservations') }}" class="nav-button {{ request()->routeIs('reservations') ? 'active' : '' }}">
            <i class="fas fa-calendar-check"></i> Onaylı Randevular
        </a>
        ...
        {{-- DÜZENLENDİ: "Bilgilerim" butonu diğerleriyle aynı tasarıma getirildi --}}
        <a href="{{ route('informations') }}" class="nav-button {{ request()->routeIs('informations') ? 'active' : '' }}">
            <i class="fas fa-user-graduate"></i> Bilgilerim
        </a>
    </div>
